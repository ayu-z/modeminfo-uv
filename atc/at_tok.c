...
char* removeConsecutiveCommas(const char *str) {
...
int at_tok_scanf(const char *in_line, const char *fmt, ...)
{
    ...
    strncpy(s_line, removeConsecutiveCommas(in_line), sizeof(s_line) - 1);
...
int at_tok_by_index_scanf(const char *line, const char *split, int index, const char *fmt, ...) 
{
    if (line == NULL || split == NULL || fmt == NULL) {
        return -1;
    }
    ...
    int tg_idx = (index >= 0) ? index : totaltoks + index + 1;

    if (tg_idx <= 0 || tg_idx > totaltoks) {
        return -2;
    }
            ...
            for (; *p; p++) {
                ...
                switch (*p) {
                    case 'd':
                        ...
                        break;
                    case 'f':
                        sscanf(tok, "%f", (float *)d);
                        break;
                    case 's':
                        *(char **)d = malloc(strlen(tok) + 1);
                        if (*(char **)d == NULL) {
                            va_end(ap);
                            return -1;
                        }
